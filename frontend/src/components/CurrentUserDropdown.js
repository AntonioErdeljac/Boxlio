    ...
    render(){
        return (
            <div style={{width: '180px', fontSize: '17px', borderStyle: 'none', marginTop: '10px', boxShadow: '0 0 10px 0 rgba(0,0,0,.1)', borderRadius: '3px', color:'rgba(0,0,0,.5)'}} className="dropdown-menu dropdown-menu-right " aria-labelledby="navbarDropdown" id="navdrop">
                <Link to={`/@${this.props.currentUser.username}`} style={{color:'rgba(0,0,0,.6)'}} className="dropdown-item"><img src={this.props.currentUser.image} height="30" style={{borderRadius: '50%', boxShadow: '0 0 20px 0 rgba(0,0,0,.1)'}} className="mx-2"/>&nbsp;@{this.props.currentUser.username}</Link>
